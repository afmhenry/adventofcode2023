...
function secondTask(filename) {
  ...
  inputAsRows.forEach((row) => {
    if (!row) return;
    let [_, _1, color] = row.split(" ");
    color = color.match(/(#[0-9a-f]+)/)[0];
    let direction = directionMap[parseInt(color.slice(-1))];
    let stepsAsNumber = parseInt(color.slice(1, -1), 16);
    console.log(direction, stepsAsNumber);
    const [x, y] = position;
    color = color.replace(/(\(|\))/g, "");
    switch (direction) {
      case "R":
  ...
  const maxX = nodes.reduce((acc, val) => {
    return acc > val[0] ? acc : val[0];
  });
  const minX = nodes.reduce((acc, val) => {
    return acc < val[0] ? acc : val[0];
  });
  const maxY = nodes.reduce((acc, val) => {
    return acc > val[1] ? acc : val[1];
  });
  const minY = nodes.reduce((acc, val) => {
    return acc < val[1] ? acc : val[1];
  });
